...
export default class ExplorerApi {
  ...
  constructor(endpoint: string, namespace: string, args: ApiArgs) {
    ...
    this.action = (async () => {
      return new ExplorerActionGenerator(
        (await this.getConfig()).contract,
        this,
      );
    })();
  }
  ...
  async getAssets(
    ...
    return await this.fetchEndpoint('/v1/assets', {
      page,
      limit,
  ...
  async countAssets(
    ...
    return await this.countEndpoint(
      '/v1/assets',
      buildDataOptions(options, data),
    );
  }
  ...
  async getCollections(
    options: CollectionApiParams = {},
    page = 1,
    limit = 100,
  ): Promise<ICollection[]> {
    return await this.fetchEndpoint('/v1/collections', {
      page,
      limit,
      ...options,
    });
  ...
  async getSchemaStats(
    collection: string,
    name: string,
  ): Promise<ISchemaStats> {
    return await this.fetchEndpoint(
      '/v1/schemas/' + collection + '/' + name + '/stats',
      {},
    );
  }
  ...
  async getSchemaLogs(
    ...
    return await this.fetchEndpoint(
      '/v1/schemas/' + collection + '/' + name + '/logs',
      { page, limit, order },
    );
  }

  async getTemplates(
    options: TemplateApiParams = {},
    page = 1,
    limit = 100,
    data: DataOptions = [],
  ): Promise<ITemplate[]> {
    return await this.fetchEndpoint('/v1/templates', {
      page,
      limit,
      ...buildDataOptions(options, data),
    });
  ...
  async getTemplateStats(
    collection: string,
    name: string,
  ): Promise<ITemplateStats> {
    return await this.fetchEndpoint(
      '/v1/templates/' + collection + '/' + name + '/stats',
      {},
    );
  }
  ...
  async getTemplateLogs(
    ...
    return await this.fetchEndpoint(
      '/v1/templates/' + collection + '/' + id + '/logs',
      { page, limit, order },
    );
  }

  async getTransfers(
    options: TransferApiParams = {},
    page = 1,
    limit = 100,
  ): Promise<ITransfer[]> {
  ...
  async getAccounts(
    ...
    page = 1,
    limit = 100,
  ): Promise<Array<{ account: string; assets: string }>> {
    return await this.fetchEndpoint('/v1/accounts', {
      page,
      limit,
      ...options,
    });
  ...
  async getAccountCollection(
    account: string,
    collection: string,
  ): Promise<IAccountCollectionStats> {
    return await this.fetchEndpoint(
      '/v1/accounts/' + account + '/' + collection,
      {},
    );
  }
  ...
  async fetchEndpoint<T>(path: string, args: any): Promise<T> {
    ...
    try {
      if (queryString.length < 1000) {
        ...
        response = await f(this.endpoint + '/' + this.namespace + path, {
          headers: {
            accept: '*.*',
