...
export default class RpcApi {
  ...
  async getAsset(owner: string, id: string, cache = true): Promise<RpcAsset> {
    if (!cache) {
  ...
  async getTemplate(
    ...
    const data = await this.queue.fetchTemplate(
      collectionName,
      templateID,
      cache,
    );

    return new RpcTemplate(
      this,
      collectionName,
      templateID,
      data,
  ...
  async getCollectionTemplates(collectionName: string): Promise<RpcTemplate[]> {
    return (await this.queue.fetchCollectionTemplates(collectionName)).map(
      (templateRow) => {
        return new RpcTemplate(
          this,
          collectionName,
          String(templateRow.template_id),
          templateRow,
          undefined,
        );
      },
  ...
  async getCollectionsSchemas(collectionName: string): Promise<RpcSchema[]> {
    return (await this.queue.fetchCollectionSchemas(collectionName)).map(
      (schemaRow) => {
  ...
  async getSchema(
    ...
    const data = await this.queue.fetchSchema(
      collectionName,
      schemaName,
      cache,
    );
  ...
  async getOffer(offerID: string, cache = true): Promise<RpcOffer> {
    if (!cache) {
  ...
  async preloadCollection(collectionName: string, cache = true): Promise<void> {
