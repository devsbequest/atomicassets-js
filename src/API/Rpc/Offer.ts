...
export default class RpcOffer {
  ...
  constructor(
    ...
    id: string,
    data?: IOfferRow,
    senderAssets?: RpcAsset[],
    receiverAssets?: RpcAsset[],
    cache = true,
  ) {
    this.id = id;
    ...
    this._senderAssets = new Promise(async (resolve, reject) => {
      if (senderAssets) {
        ...
        try {
          ...
          const inventory: IAssetRow[] =
            await this.api.queue.fetchAccountAssets(row.sender);

          return resolve(
            row.sender_asset_ids.map((assetID) => {
              const asset = inventory.find(
                (assetRow) => assetRow.asset_id === assetID,
              );
    ...
    this._recipientAssets = new Promise(async (resolve, reject) => {
      if (receiverAssets) {
        ...
        try {
          const row: IOfferRow = await this._data;
          const inventory: IAssetRow[] =
            await this.api.queue.fetchAccountAssets(row.recipient);

          return resolve(
            row.recipient_asset_ids.map((assetID) => {
              const asset = inventory.find(
                (assetRow) => assetRow.asset_id === assetID,
              );
      ...
      sender: {
        ...
        assets: await Promise.all(
          (
            ...
            typeof asset === 'string' ? asset : await asset.toObject(),
          ),
        ),
      },
      recipient: {
