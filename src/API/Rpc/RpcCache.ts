...
export default class RpcCache {
  ...
  getAsset(assetID: string, data?: IAssetRow): IAssetRow | null {
    if (data) {
      data.mutable_serialized_data = new Uint8Array(
        data.mutable_serialized_data,
      );
      data.immutable_serialized_data = new Uint8Array(
        data.immutable_serialized_data,
      );
    }
  ...
  getTemplate(
    ...
    return this.access<ITemplateRow>(
      'templates',
