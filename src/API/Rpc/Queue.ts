...
export default class RpcQueue {
  ...
  constructor(
    ...
    private readonly requestLimit: number = 4,
  ) {}
  ...
  async fetchAsset(
    ...
    return await this.fetch_single_row<IAssetRow>(
      'assets',
      owner,
      assetID,
      (data?: IAssetRow) => {
        return useCache || typeof data !== 'undefined'
          ? this.api.cache.getAsset(assetID, data)
          : null;
      },
    );
  }
  ...
  async fetchAccountAssets(account: string): Promise<IAssetRow[]> {
    const rows = await this.fetch_all_rows<IAssetRow>(
      'assets',
      account,
      'asset_id',
    );
  ...
  async fetchTemplate(
    ...
    templateID: string,
    useCache = true,
  ): Promise<ITemplateRow> {
    return await this.fetch_single_row<ITemplateRow>(
      'templates',
      collectionName,
      templateID,
      (data?: ITemplateRow) => {
        return useCache || typeof data !== 'undefined'
          ? this.api.cache.getTemplate(collectionName, templateID, data)
          : null;
      },
    );
  }
  ...
  async fetchSchema(
    ...
    return await this.fetch_single_row<ISchemaRow>(
      'schemas',
      collectionName,
      schemaName,
      (data?: ISchemaRow) => {
        return useCache || typeof data !== 'undefined'
          ? this.api.cache.getSchema(collectionName, schemaName, data)
          : null;
      },
    );
  }
  ...
  async fetchCollection(
    ...
    return await this.fetch_single_row<ICollectionRow>(
      'collections',
      this.api.contract,
      collectionName,
      (data?: ICollectionRow) => {
        return useCache || typeof data !== 'undefined'
          ? this.api.cache.getCollection(collectionName, data)
          : null;
      },
    );
  }
  ...
  async fetchCollectionSchemas(collectionName: string): Promise<ISchemaRow[]> {
    const rows = await this.fetch_all_rows<ISchemaRow>(
      'schemas',
      collectionName,
      'schema_name',
    );
    ...
    return rows.map((schema) => {
      return <ISchemaRow>(
        this.api.cache.getSchema(collectionName, schema.schema_name, schema)
      );
    });
  }

  async fetchCollectionTemplates(
    collectionName: string,
  ): Promise<ITemplateRow[]> {
    const rows = await this.fetch_all_rows<ITemplateRow>(
      'templates',
      collectionName,
      'template_id',
    );
  ...
  async preloadCollection(
    ...
    if (
      !useCache ||
      !this.preloadedCollections[collectionName] ||
      this.preloadedCollections[collectionName] + 15 * 60 * 1000 < Date.now()
    ) {
  ...
  async fetchOffer(offerID: string, useCache = true): Promise<IOfferRow> {
    return await this.fetch_single_row<IOfferRow>(
      'offers',
      this.api.contract,
      offerID,
      (data?: IOfferRow) => {
        return useCache || typeof data !== 'undefined'
          ? this.api.cache.getOffer(offerID, data)
          : null;
      },
    );
  }
      ...
      this.elements.push(async () => {
        ...
        try {
          ...
          if (resp.rows.length === 0) {
            return reject(
              new Error('Row not found for ' + JSON.stringify(options)),
            );
          }
  ...
  private async fetch_all_rows<T>(
    ...
    return new Promise(async (resolve, reject) => {
      this.elements.push(async () => {
        const resp: { more: boolean; rows: any[] } =
          await this.api.getTableRows({
            code: this.api.contract,
            scope,
            table,
